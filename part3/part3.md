...
Exercises:
  * Write a function called `all_word_pairs(string)` that given a string, returns an array of every possible pair of words. 
    * Each pair should only appear once. For example, `["cat", "yogurt", "phone"]` should return `[["cat", "yogurt"], ["cat", "phone"], ["yogurt", "phone"]]`.
...
Exercise:
  * Go back to `reverse!` and see where you can refactor to use parallel assignment.
